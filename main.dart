        ...
        children: [
          ...
          SafeArea(
            child: Padding(
              ...
              child: Column(
                children: [
                  _glassBox(height: 100, child: Center(child: Text(_trackTitle, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white), textAlign: TextAlign.center))),
                  const Spacer(),
                  _glassBox(
                    height: 300,
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            _glassButton(_isPlaying ? Icons.pause : Icons.play_arrow, () => _isPlaying ? _player.pause() : _player.play(), color: _isPlaying ? Colors.amber : Colors.white),
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void initState() {
    super.initState();
    SharedPreferences.getInstance().then((prefs) => _controller.text = prefs.getString('bridge_url') ?? '');
  }
  void _save() async {
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Container(padding: const EdgeInsets.all(15), color: Colors.white10, child: const Text("1. Create 'ZenSei_Music' in Drive.\n2. Create Apps Script.\n3. Paste code below.\n4. Deploy Web App (Anyone).\n5. Paste URL below.", style: TextStyle(color: Colors.white))),
              const SizedBox(height: 20),
              Container(padding: const EdgeInsets.all(10), color: Colors.black54, child: const SelectableText("function doGet() {\n  var f = DriveApp.getFoldersByName('ZenSei_Music').next();\n  var files = f.getFiles();\n  var list = [];\n  while (files.hasNext()) {\n    var file = files.next();\n    list.push({id: file.getId(), name: file.getName()});\n  }\n  return ContentService.createTextOutput(JSON.stringify(list)).setMimeType(ContentService.MimeType.JSON);\n}", style: TextStyle(fontFamily: 'monospace', color: Colors.greenAccent))),
              const SizedBox(height: 20),
              TextField(controller: _controller, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(filled: true, fillColor: Colors.white10, hintText: "Paste Bridge URL...")),
              const SizedBox(height: 20),
              ElevatedButton(onPressed: _save, style: ElevatedButton.styleFrom(backgroundColor: Colors.cyan, padding: const EdgeInsets.all(15)), child: const Text("SAVE", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold))),
              const SizedBox(height: 10),
              ElevatedButton(onPressed: () => Navigator.pop(context), style: ElevatedButton.styleFrom(backgroundColor: Colors.grey, padding: const EdgeInsets.all(15)), child: const Text("BACK")),
            ],
          ),
        ),
      ),
...
class _LibraryScreenState extends State<LibraryScreen> {
  ...
  Future<void> _fetchTracks() async {
    try {
      ...
      if (response.statusCode == 200) setState(() { _tracks = json.decode(response.body); _loading = false; });
    } catch (e) { setState(() => _loading = false); Fluttertoast.showToast(msg: "Check connection/URL"); }
  }
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        child: Column(
          children: [
            ...
            Expanded(child: _loading ? const Center(child: CircularProgressIndicator(color: Colors.cyan)) : ListView.builder(itemCount: _tracks.length, itemBuilder: (ctx, i) {
              final t = _tracks[i];
              return ListTile(
                leading: Container(width: 50, height: 50, color: Colors.white10, child: const Icon(Icons.music_note, color: Colors.white)),
                title: Text(t['name'], style: const TextStyle(color: Colors.white)),
                onTap: () { widget.onPlay("https://docs.google.com/uc?export=download&id=${t['id']}", t['name'], "https://images.unsplash.com/photo-1494232410401-ad00d5433cfa"); Navigator.pop(context); },
              );
            })),
          ],
        ),
